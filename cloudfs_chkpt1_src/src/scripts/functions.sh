...
{
	OPTIONS=`getopt -o s:f:a:t:S:w:c:do --long ssd-path:,fuse-path:,ssd-size:,threshold:,avg-seg-size:,rabin-window-size:,cache-size:,no-dedup,no-cache -n '$0' -- "$@"`
...
function check_cache_size()
{
  ...
  if [ -d "$CACHEDIR" ]; then
    $cache_size=$(du -s $CACHEDIR | cut -f 1)
