...
"   -/--min-seg-size    :  Desired minimum segment size for deduplication(in KB)\n"
"   -/--avg-seg-size    :  Desired average segment size for deduplication(in KB)\n"
"   -/--max-seg-size    :  Desired maximum segment size for deduplication(in KB)\n"
...
static struct option longOptionsG[] =
{
    ...
    { "min-seg-size",		required_argument,			0,  'm' },
    { "max-seg-size",		required_argument,			0,  'M' },
...
static void parse_arguments(int argc, char* argv[], 
    ...
    state->no_dedup = 0;
    state->min_seg_size = 3072;
    state->avg_seg_size = 4096;
    state->max_seg_size = 6144;
    state->rabin_window_size = 48;
    state->cache_size = 0; // Default: no cache.
    ...
    while (1) {
       ...
       case 'S': 
            ...
            break;
       case 'M':
            state->max_seg_size = atoi(optarg)*1024;
            break;
       case 'c':
    ...
    if (!((state->min_seg_size <= state->avg_seg_size) && (state->avg_seg_size <= state->max_seg_size))) {
      fprintf(stderr, "\nERROR: Segment sizes (min,avg,max) seem wrong: (%d, %d, %d)",
          state->min_seg_size, state->avg_seg_size, state->max_seg_size);
      // Usage exit
      usageExit(stderr);
    }
}
